...
import { genreNameSchema, renameGenreSchema, formatZodError } from "@/lib/validations/anime";
...
export async function renameGenre(oldName: string, newName: string): Promise<ActionResult> {
    ...
    try {
        ...
        type AnimeWithGenres = { id: number; genres: string[] | null };
        ...
        for (const anime of animesToUpdate) {
            ...
            await supabase
                .from("animes")
                .update({ genres: updatedGenres } as never)
                .eq("id", anime.id);
        }
...
export async function deleteGenre(genreName: string): Promise<ActionResult<{ updatedCount: number }>> {
    ...
    try {
        ...
        // Update each anime by removing the genre
        for (const anime of animesToUpdate) {
