            ...
            <DialogContent className="max-w-xl bg-bg-main border-white/5 p-0 overflow-hidden text-white sm:rounded-2xl shadow-2xl">
                ...
                <form action={handleSubmit} className="flex flex-col">
                    <div className="px-6 pt-12 pb-6 space-y-6">
                        {item.media_type === "tv" && (
                            <div className="space-y-3">
                                <Label className="text-xs font-semibold text-text-main/50 uppercase tracking-widest pl-1">Bölüm Yapısı</Label>
                                <div className="grid grid-cols-2 gap-3">
                                    <button type="button" onClick={() => setStructureType("seasonal")} className={cn("flex flex-col items-center justify-center gap-2 p-3 rounded-xl border transition-all duration-200", structureType === "seasonal" ? "bg-primary/10 border-primary text-primary" : "bg-white/5 border-white/5 text-text-main/50 hover:bg-white/10")}>
                                    ...
                                    <button type="button" onClick={() => setStructureType("absolute")} className={cn("flex flex-col items-center justify-center gap-2 p-3 rounded-xl border transition-all duration-200", structureType === "absolute" ? "bg-primary/10 border-primary text-primary" : "bg-white/5 border-white/5 text-text-main/50 hover:bg-white/10")}>
                        ...
                        <div className="space-y-3">
                            <Label htmlFor="title" className="text-xs font-semibold text-text-main/50 uppercase tracking-widest pl-1">Son Düzenleme (Başlık)</Label>
                    ...
                    <DialogFooter className="px-6 py-4 bg-white/5 border-t border-white/5 flex flex-col sm:flex-row gap-3">
                        <Button type="button" variant="ghost" onClick={onClose} className="h-10 text-text-main/50 hover:text-white hover:bg-white/5 rounded-lg text-sm">Vazgeç</Button>
                        <SubmitButton />
                    </DialogFooter>
                </form>
            </DialogContent>
