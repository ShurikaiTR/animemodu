    ...
    return (
        <div className="md:hidden mb-6">
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <button className="flex items-center justify-between w-full px-5 py-3.5 bg-bg-secondary/50 backdrop-blur-md border border-white/10 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all active:scale-[0.98]">
                        <span className="flex flex-col items-start leading-tight">
                            <span className="text-[10px] text-white/40 uppercase tracking-wider font-bold mb-0.5">Kategori</span>
                ...
                <DropdownMenuContent
                    align="start"
                    className="w-[calc(100vw-2rem)] min-w-[200px] bg-bg-secondary/95 backdrop-blur-xl border-white/10 p-2 rounded-2xl shadow-2xl"
                >
                    {tabs.map((tab) => (
                        <DropdownMenuItem
                            key={tab.id}
